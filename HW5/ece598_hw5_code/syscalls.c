...
//				printk("After write, %d\n",result);
	...
	/* FIXME: The SWI handler code restores all of the registers	*/
	/* Before returning.  To get our result in r0 we have	*/
	/* to push it into the place of the saved r0 on the	*/
	/* stack.						*/

	/* This breaks if you use any local vars */
	/* Just ignore it for now, replace with asm later */
